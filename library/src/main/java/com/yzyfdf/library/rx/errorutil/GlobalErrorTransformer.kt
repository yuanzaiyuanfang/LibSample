...
class GlobalErrorTransformer<T> constructor(
    ...
    override fun apply(upstream: Observable<T>): Observable<T> = upstream.flatMap {
    ...
    override fun apply(upstream: Completable): Completable = upstream.onErrorResumeNext {
        onErrorResumeNext(it).ignoreElements()
    }.retryWhen(FlowableRetryDelay(onErrorRetrySupplier))
        .doOnError(onErrorConsumer)

    override fun apply(upstream: Flowable<T>): Flowable<T> = upstream.flatMap {
    ...
    override fun apply(upstream: Maybe<T>): Maybe<T> = upstream.flatMap {
    ...
    override fun apply(upstream: Single<T>): Single<T> = upstream.flatMap {
