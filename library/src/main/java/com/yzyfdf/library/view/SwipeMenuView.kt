...
class SwipeMenuView @JvmOverloads constructor(
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        mScaleTouchSlop = ViewConfiguration.get(context)
            .scaledTouchSlop
        ...
        //左滑右滑的开关,默认左滑打开菜单
        isLeftSwipe = true
        val ta = context.theme.obtainStyledAttributes(attrs, R.styleable.SwipeMenuView,
                defStyleAttr, 0)
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val measureMatchParentChildren = View.MeasureSpec.getMode(
            ...
            if (childView.visibility != View.GONE) {
                if (i == 0) { //第一个子View是内容 宽度设置为全屏
                    ...
                    if (isLeftSwipe) {
                        ...
                        childView.layout(right - childView.measuredWidth, paddingTop, right,
                                paddingTop + childView.measuredHeight)
            ...
            when (ev.action) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    if (!iosInterceptFlag) {
                        ...
                        if (Math.abs(gap) > 10 || Math.abs(
                                        scrollX) > 10) { //2016 09 29 修改此处，使屏蔽父布局滑动更加灵敏，
                            parent.requestDisallowInterceptTouchEvent(true)
                        }
